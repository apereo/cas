...
<c:if test="${fn:length(services) eq 0}">
    <div id="msg" class="errors"><p><spring:message code="management.services.service.warn"
                                                    arguments="${defaultServiceUrl}"/></p></div>
</c:if>

<c:if test="${not empty param.status}">
    <div id="msg" class="success"><spring:message code="management.services.status.${param.status}"
                                                  arguments="${param.serviceName}"/></div>
</c:if>
    ...
    <table id="scrollTable" class="scrollTable highlight large">
        ...
        <c:forEach items="${services}" var="service" varStatus="status">
            <tr id="row${status.index}"${param.id eq service.id ? ' class="added"' : ''}>
                ...
                <td class="ac td3"><img src="images/${service.enabled}.gif"
                                        alt="${service.enabled ? 'Enabled' : 'Disabled'}"/></td>
                <td class="ac td4"></td>
                <td class="ac td5"><img src="images/${service.ssoEnabled}.gif"
                                        alt="${service.ssoEnabled ? 'SSO Enabled' : 'SSO Disabled'}"/></td>
                <td class="ac td6"><img src="images/${service.anonymousAccess}.gif"
                                        alt="${service.anonymousAccess ? 'Anonyous Access Enabled' : 'Anonyous Access Disabled'}"/>
                </td>
                <td class="ac td7">${service.usernameAttribute}</td>
                <td class="ac td8">${service.evaluationOrder}</td>
                <td class="td9" id="edit${status.index}"><a href="edit.html?id=${service.id}"
                                                            class="edit"><spring:message
                        code="management.services.manage.action.edit"/></a></td>
                <td class="td10" id="delete${status.index}"><a href="#" class="del"
                                                               onclick="swapButtonsForConfirm('${status.index}','${service.id}'); return false;"><spring:message
                        code="management.services.manage.action.delete"/></a></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
</div>
<div class="add"><a href="add.html"><span style="text-transform: lowercase;"><spring:message
        code="addServiceView"/></span></a></div>
