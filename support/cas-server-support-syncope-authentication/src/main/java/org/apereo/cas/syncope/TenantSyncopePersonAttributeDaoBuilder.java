...
public class TenantSyncopePersonAttributeDaoBuilder implements TenantPersonAttributeDaoBuilder {
    ...
    public List<? extends PersonAttributeDao> build(final TenantDefinition tenantDefinition) {
        ...
        if (bindingContext.isBound() && bindingContext.containsBindingFor(SyncopePrincipalAttributesProperties.class)) {
            val casProperties = bindingContext.value();
            val syncope = casProperties.getAuthn().getAttributeRepository().getSyncope();
            val repositories = SyncopeUtils.newPersonAttributeDaos(syncope);
