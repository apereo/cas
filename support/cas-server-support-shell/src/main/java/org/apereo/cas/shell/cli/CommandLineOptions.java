...
public class CommandLineOptions {
     ...
     */
    public static final Option OPTION_SHELL = Option.builder("sh").longOpt("shell")
        .desc("Launch into a CAS interactive shell to execute commands. Activating this option will disable "
            + "basic CLI capabilities and allows you to interactively query the CAS configuration metadata server and more.").build();

    /**
     * Command line option that indicates a property.
     */
    public static final Option OPTION_PROPERTY = Option.builder("p").longOpt("property")
        .hasArg().desc("Indicate the property name (i.e. cas.server.name). A regular expression pattern is an acceptable value, depending on function and usage.").build();

    /**
     * Command line option that indicates a summary.
     */
    public static final Option OPTION_SUMMARY = Option.builder("su").longOpt("summary")
        .desc("Display a compact version of the query results; Summarize output.").build();

    /**
     * Command line option that indicates a subject.
     */
    public static final Option OPTION_SUBJECT = Option.builder("sub").hasArg().longOpt("subject")
    ...
    public static final Option OPTION_STRICT_MATCH = Option.builder("sm")
        .longOpt("strict-match").desc("Control whether pattern matching should be done in strict mode which means "
            + "the matching engine tries to match the entire region for the query.").build();
