...
public class DuoSecurityAdminApiEndpointTests {
    ...
    public void verifyOperation() throws Exception {
        ...
        val resource = IOUtils.toString(new ClassPathResource("duoAdminApiResponse-user.json").getInputStream(), StandardCharsets.UTF_8);
        try (val webServer = new MockWebServer(8443,
            new ByteArrayResource(resource.getBytes(StandardCharsets.UTF_8), "Output"), HttpStatus.OK)) {
            webServer.start();
            val result = endpoint.getUser("casuser", DuoSecurityMultifactorAuthenticationProperties.DEFAULT_IDENTIFIER);
