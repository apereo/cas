...
public class EditableAttributeFormAction extends AbstractAction {
    ...
    public EditableAttributeFormAction(final EditableAttributeValueRepository valueRepository,
            final EditableAttributeRepository attributeRepository, final EditableAttributeValueValidator validator) {
     ...
     *            the message context
     * @return success if attributes are valid.
     *         {@link #EVENT_ID_ATTRIBUTE_VALUES_NEEDED} otherwise.
     */
    public Event verify(final RequestContext requestContext, final Credential credential,
     ...
     */
    public Event submit(final RequestContext requestContext, final Credential credential,
            final MessageContext messageContext) {
    ...
    /**
     * Utility method to populate all possible flow variables to avoid duplication
     * in other methods.
     * 
     * @param requestContext
     *            Request context
    ...
    private void populateFlowScope(final RequestContext requestContext, Credential credential) {
        ...
        final Map<String, String> existingValues = principalAndValues.getRight();
        ...
        requestContext.getFlowScope().put(FLOW_STORAGE_INPUT_VALUES,
                processInputValues(attributes, existingValues, request));
     ...
     * @param request
     *            request containing form values
     * @return actual values containing secret data and no placeholders
     */
    private static Map<String, String> processInputValues(final List<EditableAttribute> attributes,
            final Map<String, String> existingValues, final HttpServletRequest request) {
        final HashMap<String, String> inputValues = new HashMap<>();
    ...
    /**
     * Preserve secrecy of password inputs by replacing them with a known
     * placeholder.
     * 
     * @param attributes
     *            Attribute list
     ...
     */
    private static Map<String, String> sanitizeOutputValues(final List<EditableAttribute> attributes,
            final Map<String, String> existingValues) {
        final HashMap<String, String> outputValues = new HashMap<>();
        attributes.forEach(attr -> {
            final String id = attr.getId();
            final String existingValue = existingValues.get(id);
