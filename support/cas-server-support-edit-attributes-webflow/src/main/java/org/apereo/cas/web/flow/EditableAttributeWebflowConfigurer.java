...
/**
 * The {@link EditableAttributeWebflowConfigurer} is responsible for
 * adjusting the CAS webflow context for editable attribute integration.
 *
...
public class EditableAttributeWebflowConfigurer extends AbstractCasWebflowConfigurer {
    ...
    private EvaluateAction createEditableAttributesAction(final String actionId) {
        return createEvaluateAction("editableAttributesFormAction."
                + actionId + "(flowRequestContext, flowScope.credential, messageContext)");
    }

    private void createSubmitActionState(final Flow flow) {
        final ActionState editableAttributesSavedAction = createActionState(flow, EDITABLE_ATTRIBUTES_SAVED_ACTION, createEditableAttributesAction("submit"));

        final String target = getRealSubmissionState(flow).getTransition(CasWebflowConstants.TRANSITION_ID_SUCCESS).getTargetStateId();
        editableAttributesSavedAction.getTransitionSet().add(createTransition(CasWebflowConstants.TRANSITION_ID_SUCCESS, target));
        editableAttributesSavedAction.getTransitionSet().add(createTransition(CasWebflowConstants.TRANSITION_ID_ERROR, CasWebflowConstants.STATE_ID_INIT_LOGIN_FORM));
    }
    ...
    private void createVerifyActionState(final Flow flow) {
        final ActionState actionState = createActionState(flow, STATE_ID_ATTRIBUTES_CHECK, createEditableAttributesAction("verify"));

        final String target = getRealSubmissionState(flow).getTransition(CasWebflowConstants.TRANSITION_ID_SUCCESS).getTargetStateId();
        actionState.getTransitionSet().add(createTransition(CasWebflowConstants.TRANSITION_ID_SUCCESS, target));
        actionState.getTransitionSet().add(createTransition(EditableAttributeFormAction.EVENT_ID_ATTRIBUTE_VALUES_NEEDED, EDITABLE_ATTRIBUTES_VIEW));
    }
}
