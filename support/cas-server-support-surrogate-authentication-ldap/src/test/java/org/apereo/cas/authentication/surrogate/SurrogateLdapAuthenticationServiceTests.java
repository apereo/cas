...
class SurrogateLdapAuthenticationServiceTests {
    private static final int LDAP_PORT = 10389;

    private static void bootstrap(final String user, final String adminUser) throws Exception {
        @Cleanup
        ...
        val ldif = IOUtils.toString(new ClassPathResource("ldif/ldap-surrogate.ldif").getInputStream(), StandardCharsets.UTF_8)
            .replace("$user", user).replace("$admin", adminUser);
        LdapIntegrationTestsOperations.populateEntries(
            localhost,
    ...
    class SingleLdapSourceTests extends BaseSurrogateAuthenticationServiceTests {
        ...
        @BeforeAll
        static void bootstrapLdap() throws Exception {
            bootstrap(USER, ADMIN_USER);
        }
    ...
    class MultiLdapSourceTests extends BaseSurrogateAuthenticationServiceTests {
        private static final String USER = RandomUtils.randomAlphabetic(10);
