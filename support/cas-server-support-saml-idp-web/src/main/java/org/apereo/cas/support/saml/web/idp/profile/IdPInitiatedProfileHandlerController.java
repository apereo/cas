...
public class IdPInitiatedProfileHandlerController extends AbstractSamlProfileHandlerController {
    ...
    protected void handleIdPInitiatedSsoRequest(final HttpServletResponse response,
        ...
        final var providerId = CommonUtils.safeGetParameter(request, SamlIdPConstants.PROVIDER_ID);
        ...
        final var registeredService = verifySamlRegisteredService(providerId);
        final var adaptor = getSamlMetadataFacadeFor(registeredService, providerId);
        ...
        if (StringUtils.isBlank(shire)) {
            ...
            @NonNull
            final var acs = facade.getAssertionConsumerService(SAMLConstants.SAML2_POST_BINDING_URI);
        ...
        final var target = CommonUtils.safeGetParameter(request, SamlIdPConstants.TARGET);

        // A timestamp to help with stale request detection.
        final var time = CommonUtils.safeGetParameter(request, SamlIdPConstants.TIME);

        final var builder = (SAMLObjectBuilder) configBean.getBuilderFactory().getBuilder(AuthnRequest.DEFAULT_ELEMENT_NAME);
        final var authnRequest = (AuthnRequest) builder.buildObject();
        authnRequest.setAssertionConsumerServiceURL(shire);

        final var isBuilder = (SAMLObjectBuilder) configBean.getBuilderFactory().getBuilder(Issuer.DEFAULT_ELEMENT_NAME);
        final var issuer = (Issuer) isBuilder.buildObject();
        issuer.setValue(providerId);
        authnRequest.setIssuer(issuer);

        authnRequest.setProtocolBinding(SAMLConstants.SAML2_POST_BINDING_URI);
        final var pBuilder = (SAMLObjectBuilder) configBean.getBuilderFactory().getBuilder(NameIDPolicy.DEFAULT_ELEMENT_NAME);
        final var nameIDPolicy = (NameIDPolicy) pBuilder.buildObject();
