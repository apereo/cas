...
<!--
	...
	function redirectTo(URL) {
		window.location = URL ;
			...
			<c:if test="${expireDays == 0}">
				alert('<spring:message code="screen.warnpass.heading.today" />');
			</c:if>
			<c:if test="${expireDays == 1}">
				alert('<spring:message code="screen.warnpass.heading.tomorrow" />');
			</c:if>
			<c:if test="${expireDays > 1}">
				alert('<spring:message code="screen.warnpass.heading.other" arguments="${expireDays}" />');
			</c:if>
//-->
