    ...
    public static PasswordEncoder newPasswordEncoder(final PasswordEncoderProperties properties,
        ...
        return switch (encoderType) {
            case DEFAULT -> {
                ...
                yield new DefaultPasswordEncoder(properties.getEncodingAlgorithm(), properties.getCharacterEncoding());
            }
            case STANDARD -> {
                LOGGER.debug("Creating standard password encoder with the secret defined in the configuration");
                yield new StandardPasswordEncoder(properties.getSecret());
            }
            case ARGON2 -> new Argon2PasswordEncoder(properties.getStrength(), properties.getHashLength(),
            ...
            case BCRYPT -> {
                ...
                if (StringUtils.isBlank(properties.getSecret())) {
                    ...
                    yield new BCryptPasswordEncoder(properties.getStrength());
                }
                LOGGER.debug("Creating BCRYPT encoder with secret");
                yield new BCryptPasswordEncoder(properties.getStrength(), RandomUtils.getNativeInstance());
            }
            case SCRYPT -> {
                LOGGER.debug("Creating SCRYPT encoder");
                yield new SCryptPasswordEncoder(DEFAULT_CPU_COST, DEFAULT_MEMORY_COST, DEFAULT_PARALLELISM,
            ...
            case SSHA -> {
                ...
                yield new LdapShaPasswordEncoder();
            }
            case PBKDF2 -> {
                ...
                yield new Pbkdf2PasswordEncoder(properties.getSecret(),
            ...
            case GLIBC_CRYPT -> {
                ...
                LOGGER.debug(msg);
                yield new GlibcCryptPasswordEncoder(properties.getEncodingAlgorithm(), properties.getStrength(), properties.getSecret());
            }
            default -> {
                LOGGER.trace("No password encoder shall be created given the requested encoder type [{}]", type);
                yield NoOpPasswordEncoder.getInstance();
            }
        };
    }
}
