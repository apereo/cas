...
/**
 * This controller returns an access token which is the CAS granting ticket
 * according to the service and code (service ticket) given.
 *
 * @author Jerome Leleu
 * @since 3.5.0
 */
...
public final class OAuth20AccessTokenController extends BaseOAuthWrapperController {
	...
	protected ModelAndView internalHandleRequest(final String method, final HttpServletRequest request,
		...
		// In more detail, we needed to prevent this method from deleting ST for
		// we need it to create PT.
		// ST is deleted in another class method once PT is created
		// successfully.
		// ticketRegistry.deleteTicket(serviceTicket.getId());

		final OAuthRegisteredService registeredService = OAuthUtils.getRegisteredOAuthService(this.servicesManager,
				clientId);
		...
		final int expires = (int) (this.timeout
				- TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis() - ticketGrantingTicket.getCreationTime()));
		final String text = String.format("%s=%s&%s=%s", OAuthConstants.ACCESS_TOKEN, accessTokenEncoded,
	...
	/**
	 * Verify access token request by reviewing the values of client id,
	 * redirect uri, client secret, code, etc.
	 *
	 * @param response
	 *            the response
	 * @param redirectUri
