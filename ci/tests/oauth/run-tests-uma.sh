...
gradle="./gradlew $@"
gradleBuild=""
gradleBuildOptions="--build-cache --configure-on-demand --no-daemon -DtestCategoryType=UMA "

echo -e "***********************************************"
echo -e "Gradle build started at `date`"
echo -e "***********************************************"

gradleBuild="$gradleBuild testUma jacocoRootReport -x test -x javadoc -x check \
    --parallel \
...
if [ -z "$gradleBuild" ]; then
    ...
    if [ $retVal == 0 ]; then
        ...
        bash <(curl -s https://codecov.io/bash) -F OAUTHUMA
