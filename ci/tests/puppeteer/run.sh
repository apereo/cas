...
function readDependencySet() {
  local filter="$1"
  jq -j "${filter} | if type == \"array\" then join(\",\") elif type == \"null\" then empty else . end" "${config}"
}

function dependenciesForInstance() {
  local instance="$1"
  local commonDependencies
  local instanceDependencies
  commonDependencies=$(readDependencySet ".dependencies // empty")
  instanceDependencies=$(readDependencySet ".instance${instance}.dependencies // empty")

  if [[ -n "${commonDependencies}" && -n "${instanceDependencies}" ]]; then
    echo "${commonDependencies},${instanceDependencies}"
  elif [[ -n "${instanceDependencies}" ]]; then
    echo "${instanceDependencies}"
  else
    echo "${commonDependencies}"
  fi
}

function casServerArtifactForInstance() {
  local instance="$1"
  if [[ "${NATIVE_BUILD}" == "true" || "${NATIVE_RUN}" == "true" ]]; then
    if [[ ${instances} -gt 1 ]]; then
      echo "$PWD/cas-instance-${instance}"
    else
      echo "${targetArtifact}"
    fi
  elif [[ ${instances} -gt 1 ]]; then
    echo "$PWD/cas-instance-${instance}.${projectType}"
  else
    echo "$PWD/cas.${projectType}"
  fi
}

function dockerImageNameForInstance() {
  local instance="$1"
  if [[ ${instances:-1} -gt 1 ]]; then
    echo "cas-${scenarioName}-${instance}"
  else
    echo "cas-${scenarioName}"
  fi
}
...
function buildAndRun() {
  ...
  if ! [[ "${instances}" =~ ^[0-9]+$ ]] || [[ ${instances} -lt 1 ]]; then
    printred "Invalid number of CAS instances defined: ${instances}"
    exit 1
  fi
  ...
  instanceDependencyCount=$(jq -j '[to_entries[] | select(.key | test("^instance[0-9]+$")) | select(.value.dependencies? != null)] | length' "${config}")
  ...
  if [[ "${RERUN}" != "true" ]]; then
    for ((c = 1; c <= instances; c++)); do
      ...
      if [[ "${REBUILD}" == "true" ]]; then
        ...
        printgreen "Building CAS instance #${c} found in $PWD for dependencies [${dependencies}] with flags [${FLAGS}]"
...
-DcasModules="${dependencies}" --no-watch-fs --max-workers=8 ${BUILDFLAGS:+ $BUILDFLAGS}")
  ...
  if [[ "${buildDockerImage}" == "true" ]]; then
    for ((c = 1; c <= instances; c++)); do
      ...
      docker images --filter "reference=${dockerImageName}"
      printgreen "Built Docker image ${dockerImageName}"
    done
  fi
  ...
  if [[ "${RERUN}" != "true" && ("${NATIVE_BUILD}" == "false" || "${NATIVE_RUN}" == "true") ]]; then
    ...
    for ((c = 1; c <= instances; c++)); do
      dependencies="${casServerDependencies[$c]}"
      casArtifactToRun="${casServerArtifacts[$c]}"
      ...
      if [[ "${INITONLY}" == "false" ]]; then
        ...
        if [[ "${launchEnabled}" == "true" ]]; then
          if [[ "${NATIVE_RUN}" == "true" ]]; then
            ...
            dockerHostJavaDebugPort=$((5005 + c - 1))
            dockerHostHttpPort=$((8080 + c - 1))
            printcyan "Launching docker container ${dockerImageName}:latest"
            docker run -d --rm \
            ...
            if [[ "${aotEnabled}" == "true" && "${serverType:-external}" != "external" ]]; then
              ...
              aotWorkingDirectory="${PWD}"
              aotArchiveToExtract="${casArtifactToRun}"
              if [[ ${instances} -gt 1 ]]; then
                aotWorkingDirectory="${PWD}/cas-instance-${c}-aot"
                rm -rf "${aotWorkingDirectory}" 2>/dev/null
                mkdir -p "${aotWorkingDirectory}"
                aotArchiveToExtract="${aotWorkingDirectory}/cas.${projectType}"
                cp "${casArtifactToRun}" "${aotArchiveToExtract}"
                if [ $? -ne 0 ]; then
                  printred "Unable to copy CAS artifact ${casArtifactToRun} for AOT extraction."
                  exit 1
                fi
              fi
              aotCasDirectory="${aotWorkingDirectory}/cas"
              rm -rf "${aotCasDirectory}" 2>/dev/null
              mkdir -p "${aotWorkingDirectory}"
              printcyan "Extracting CAS to ${aotCasDirectory}"
              aotArgs=${runArgs//suspend=y/suspend=n}
              aotArgs=${aotArgs//address=*:$DEBUG_PORT/address=*:15555}
              (cd "${aotWorkingDirectory}" && java ${aotArgs} -Djarmode=tools -jar "${aotArchiveToExtract}" extract >/dev/null 2>&1)
              if [ $? -ne 0 ]; then
                printred "Failed to extract CAS artifact ${aotArchiveToExtract} for AOT."
                exit 1
              fi
              if [[ ! -f "${aotCasDirectory}/cas.${projectType}" ]]; then
                printred "CAS AOT extraction did not produce ${aotCasDirectory}/cas.${projectType}."
                exit 1
              fi
              printcyan "Launching CAS from ${aotCasDirectory}/cas.${projectType} to perform a training run"
              java ${aotArgs} -XX:AOTCacheOutput=${aotCasDirectory}/cas.aot -Dspring.context.exit=onRefresh -jar "${aotCasDirectory}/cas.${projectType}" >/dev/null 2>&1
              printcyan "Generated archive cache file ${aotCasDirectory}/cas.aot"
  ...
  if [[ "${RERUN}" != "true" ]]; then
    ...
    if [[ "${buildDockerImage}" == "true" ]]; then
      for ((c = 1; c <= ${instances:-1}; c++)); do
        dockerImageName=$(dockerImageNameForInstance "$c")
        printgreen "Stopping CAS Docker container ${dockerImageName}..."
        docker stop "${dockerImageName}" >/dev/null 2>&1
        docker rm "${dockerImageName}" >/dev/null 2>&1
      done
    fi
  fi
}
