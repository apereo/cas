...
if [[ "${NATIVE_BUILD}" == "false" ]]; then
  ...
  project="native"
  echo "Project type is chosen to be ${project}"
fi
...
if [[ "${REBUILD}" == "true" && "${RERUN}" != "true" ]]; then
  ...
  echo "Target artifact generated by the builds: ${targetArtifact}"
  ...
  if [[ "${NATIVE_BUILD}" == "true" ]]; then
    BUILD_TASKS="${BUILD_TASKS} :webapp:cas-server-webapp-${project}:nativeCompile"
  fi
