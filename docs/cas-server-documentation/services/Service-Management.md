...
| `importRegisteredServices`    | Import service definitions into the [CAS service registry](Service-Management.html). The request body is expected to be the service definition itself. For bulk imports, you may also specify a zip file containing service definition files with the content-type of `application/octet-stream`.
