...
- `DefaultOAuth20ProofOfPossessionValidator` is built on Nimbus's `DPoPTokenRequestVerifier`, which is the token-endpoint verifier: it checks `htm`, `htu` and `jti` but never `ath`. Resource requests want `DPoPProtectedResourceRequestVerifier`. The credential endpoint does not call the validator at all. Do not assume a DPoP-bound token is actually sender-constrained once it leaves the token endpoint.
